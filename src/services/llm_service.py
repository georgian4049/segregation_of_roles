...
class LLMProvider(ABC):
    ...
    @abstractmethod
    async def generate(self, prompt: str, max_tokens: int, profile: UserViolationProfile | None = None) -> str:
...
class MockLLMProvider(LLMProvider):
    ...
    async def generate(self, prompt: str, max_tokens: int, profile: UserViolationProfile | None = None) -> str:
        ...
        violated_policy_ids = ", ".join([p.policy_id for p in profile.violated_policies])
        conflicting_roles = list(profile.conflicting_role_set)
        role_to_revoke = conflicting_roles[0] if conflicting_roles else "a_conflicting_role"
    ...
    async def generate(self, prompt: str, max_tokens: int, profile: UserViolationProfile | None = None) -> str:
        ...
        try:
            if "amazon.titan" in self.model_id:
                ...
                raise NotImplementedError(f"Model format for {self.model_id} not implemented.")
            ...
            if "amazon.titan" in self.model_id:
                ...
                raise NotImplementedError(f"Response parsing for {self.model_id} not implemented.")

        except ClientError as e:
            if e.response['Error']['Code'] == 'ValidationException':
                 logger.error(f"Bedrock ValidationException: {e}. Check if model is enabled in AWS.")
...
class LLMService:
    ...
    def _init_provider(self) -> LLMProvider:
        if settings.use_mock_llm:
            ...
            logger.info(f"Using Bedrock provider with model: {settings.bedrock_model_id}")
            try:
                ...
                logger.error("Bedrock initialization failed, falling back to mock: %s", exc)
                provider = self._fallback_provider
        else:
            logger.error(f"Unknown LLM provider '{settings.llm_provider}', falling back to mock.")
            provider = self._fallback_provider

        self.status = {
            "provider": provider.__class__.__name__,
            "using_mock": isinstance(provider, MockLLMProvider),
            "fallback": provider is self._fallback_provider and not (settings.use_mock_llm or settings.llm_provider == "mock"),
    ...
    def _parse_and_validate_response(
        ...
        try:
            ...
            if not (risk and action and rationale):
                 raise ValueError(f"JSON object is missing required keys.")
            
            return risk, action, rationale

        except (json.JSONDecodeError, KeyError, ValueError) as e:
            logger.warning(f"LLM JSON parsing failed. Error: {e}. Response was: {response_text}")
    ...
    async def generate_user_remediation(
        ...
        for attempt in range(max_retries):
            try:
                ...
                risk, action, rationale = self._parse_and_validate_response(response_text)
